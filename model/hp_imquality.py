...
# HP_LEARNINGRATE = hp.HParam('dropout', hp.RealInterval(0.001, 0.002))
# HP_TARGETSIZE = hp.HParam('num_units', hp.Discrete([256, 512]))
# HP_BATCHSIZE = 32, 64, 128, 256
...
def train_test_model(hparams):
    ...
    ############# evaluate with test
    y_true = testdf['im_quality']
    y_pred_detail = model.predict_generator(test_generator)
    y_pred = [str(round(y_pred_i[0])) for y_pred_i in y_pred_detail]
    print(y_pred)
